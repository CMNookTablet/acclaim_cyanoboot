...
void dev_print (block_dev_desc_t *dev_desc)
{
    ...
    defined(CONFIG_FASTBOOT_PARTITION)
...
void init_part (block_dev_desc_t * dev_desc)
{
	...
	/* Check Fastboot partitions first, since that's the mechanism used
	   for GPT partitions, which take priority over the protective DOS MBR
	   partition which would be detected by test_part_dos() below */
	if (test_part_fastboot(dev_desc) == 0) {
		dev_desc->part_type = PART_TYPE_FASTBOOT;
		return;
	}
#endif
...
#ifdef CONFIG_FASTBOOT_PARTITION
	case PART_TYPE_FASTBOOT:
		if (get_partition_info_fastboot(dev_desc,part,info) == 0) {
			PRINTF ("## Valid Fastboot partition found ##\n");
			return (0);
		}
		break;
#endif
...
#ifdef CONFIG_FASTBOOT_PARTITION
	case PART_TYPE_FASTBOOT:
	    PRINTF ("## Testing for a valid fastboot partition ##\n");
	    print_part_header ("Fastboot", dev_desc);
	    print_part_fastboot (dev_desc);
	    return;
#endif
	}
...
#else
# error no partition type configured!
#endif
